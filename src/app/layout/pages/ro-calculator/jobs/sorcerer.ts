...
export class Sorcerer extends CharacterBase {
  ...
  protected readonly _atkSkillList: AtkSkillModel[] = [
    {
      ...
      formular: (params: AtkSkillFormulaInput<{ extra: any }>): number => {
        ...
        return (skillLevel + 2) * totalInt * (baseLevel / 100);
      },
    },
    {
      name: 'Earth Grave',
    ...
    },
    {
      ...
      formular: (params: AtkSkillFormulaInput<{ extra: any }>): number => {
        ...
        return (((skillLevel + 2) * totalInt) / 2 + (strikingLvl + endowLvl) * 150) * (baseLevel / 100);
      },
    },
  ];
  ...
  override setAdditionalBonus(params: InfoForClass): Record<string, any> {
    ...
    const { masteryAtks } = this.getSkillBonusAndName();
    let atkPercent = 0;
    let aspdPercent = 0;
    for (const [_skillName, bonus] of Object.entries(masteryAtks)) {
      atkPercent += bonus[`${typeName}_atk`] || 0;
      aspdPercent += bonus[`${typeName}_aspdPercent`] || 0;
    }
    console.log({ typeName, masteryAtks, atkPercent, aspdPercent });
